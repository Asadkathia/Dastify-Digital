...
export function buildConverterPrompt(html: string, pageName: string): AIMessage[] {
  const system = [
    ...
    '6. Replace <img> with Next.js <Image> from "next/image" where possible. For placeholder boxes keep a <div> with the same dimensions.',
    ...
    '- Image placeholder boxes (.iph) should keep their emoji icon, label, and pixel dimensions.',
