...
export function buildConverterPrompt(html: string, pageName: string): AIMessage[] {
  ...
  const system = [
    ...
    '  "globalsCssAdditions": "...",',
    '  "editorRegistry": "...",',
    '  "sections": [{ "key": "nav", "label": "Navbar", "icon": "🧭", "className": "nav" }, ...] // NEVER include a footer entry',
    '}',
    '',
    '─── COMPONENT RULES ───',
    '1. Split the page into logical section components — one file per section (Hero, Navbar, Services, Results, WhySection, Process, Cta, etc.). NEVER generate a footer component — see rule 1a.',
    '1a. NEVER generate a footer component. The site injects a global SiteFooter automatically on every route. Strip any <footer> element from the source HTML entirely — do not convert it.',
    ...
    '  - imports { SiteFooter } from "@/components/SiteFooter" (named export)',
    '  - imports { getFooter } from "@/lib/cms/queries" (named export)',
    '  - accepts content from a hardcoded defaultContent object (passed as props to each component)',
    '  - fetches footer data: const footer = await getFooter()',
    '  - renders <SiteFooter footer={footer} /> AFTER all section components (outside <main>)',
    ...
    'PageContent MUST NEVER include a "footer" field at the top level. Footer data is global and managed separately via the Payload Footer global — the SiteFooter component reads it automatically.',
    ...
    'Use semantic fields like `titleTag` when a heading level should be selectable by the editor. When rendering a dynamic tag from titleTag, cast it as a string literal union — NEVER use `keyof JSX.IntrinsicElements` (the JSX namespace is not globally available). Example: `const TitleTag = (data.titleTag || "h2") as "h1" | "h2" | "h3" | "h4";`',
    ...
    'NEVER include a footer entry in the sections array. NEVER import a footer component. The route renders the global SiteFooter outside this registry — do not register it.',
    'editorRegistry must be a complete TypeScript source string for editor-registry.ts. It must:',
    '1. Import every section component (NEVER a footer component): import XNavbar from "./components/XNavbar"; etc.',
