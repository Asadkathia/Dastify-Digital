...
async function resolveSourceFile(fileName: string): Promise<string> {
...
async function main(): Promise<void> {
  ...
  const localMedia = runSqliteJson<LocalMediaRow>(
  ...
  const remoteByFilename = new Map(
    remoteExisting.docs
      .filter((doc): doc is typeof doc & { filename: string; id: number | string } => typeof doc.filename === 'string')
